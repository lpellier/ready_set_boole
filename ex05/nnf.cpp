...
public:
	...
	Node(const Node & src) {
		...
		this->left = new Node(*(src.left));
		this->right = new Node(*(src.right));
	}
};
...
void rewriteTree(Node * cur) {
	...
	if (cur->left && cur->right && cur->value == '>') {
		...
		Node * left_tmp = cur->left;
		Node * right_tmp = cur->right;
		
		cur->value = '&';

		cur->left = new Node('>');
		cur->left->left = new Node(right_tmp->value);
		cur->left->right = new Node(left_tmp->value);
		
		cur->right = new Node('>');
		cur->right->left = new Node(left_tmp->value);
		cur->right->right = new Node(right_tmp->value);
		free_nodes(left_tmp);
...
std::string	negation_normal_form(const std::string & formula) {
	...
	// ? read the tree and reverse it into a string -> result
